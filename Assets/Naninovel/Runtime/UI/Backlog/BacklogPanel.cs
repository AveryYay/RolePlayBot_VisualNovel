...
namespace Naninovel.UI
{
    public class BacklogPanel : CustomUI, IBacklogUI, ILocalizableUI
    {
        ...
        public virtual string GetFullLogTextPlain()
        {
            var result = new System.Text.StringBuilder();

            foreach (var msgUI in messages)
            {
                var state = msgUI.GetState();
                if (state.Text == null)
                    continue;

                string rawText = (string) state.Text;
                ...
                foreach (var line in lines)
                {
                    if (line.Contains("<br>"))
                    {
                        var choices = line.Split(new[] { "    " }, StringSplitOptions.RemoveEmptyEntries);
                        foreach (var choice in choices)
                        {
                            if (choice.Contains("<b>"))
                            {
                                var cleanedChoice = Regex.Replace(choice, "<.*?>", string.Empty).Trim();
                                result.AppendLine($"player chose: {cleanedChoice}");
                            }
                        }
                    ...
                    else
                    {
                        ...
                        result.AppendLine(state.GetAuthor() + ": " + line);
                    }
